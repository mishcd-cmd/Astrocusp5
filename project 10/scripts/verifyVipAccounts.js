// scripts/verifyVipAccounts.js
// Usage examples:
//   node scripts/verifyVipAccounts.js --email="mish.cd@gmail.com" --show
//   node scripts/verifyVipAccounts.js --email="mish.cd@gmail.com" --set-hemisphere="Southern"
//   node scripts/verifyVipAccounts.js --email="user@example.com" --set-hemisphere="Northern" --enforce-vip
//
// ENV required:
...
function logSupabaseError(where, err) {
...
async function resilientUpsert(supabase, table, payload, conflictTarget) {
  // Attempt 1
...
async function main() {
  ...
  const user = all.data.users.find((u) => (u.email || '').toLowerCase() === email);
  ...
  const payload = {
    // Use user_id as the primary key since that's what your schema uses
    user_id: user.id,
